...
import { Client, Identity, ThreadID, Query, Where } from '@textile/hub';
      ...
      const currUserConnectionState =
        ...
        peers[id].on('signal', (data) => {
          ...
          async function asyncWrapper() {
            const tok = await client.getToken(identity); // auth
            ...
            if (tableDb.name) { 
              ...
              const connectDetail: DbConnectDetail = { from: userID, to: id, data: JSON.stringify(data)};
        ...
        window.setInterval(() => {
          async function asyncWrapper() {
            ...
            if (tableDb.name) { 
              ...
              const query: Query = new Where('from').eq(id).and('to').eq(userID);
              const foundData: DbConnectDetail[] = await client.find(threadId, 'connectDetail', query);
